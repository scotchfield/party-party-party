...
const { createPartyImage } = require('../index');
...
function run(launchFilePath) {
  ...
  if (args._.length !== 2) {
    console.log(
      'Usage: ' + (launchFilePath || __filename) + ' input.png output.gif'
    );
    process.exit(-1);
  }
  ...
  createPartyImage({
    ...
    bounceSpeed: parseFloat(args.bounceSpeed),
    colorSpeed: parseFloat(args.colorSpeed),
    noParty: args.noParty === 'true',
    backgroundParty: args.backgroundParty === 'true',
  });
}
